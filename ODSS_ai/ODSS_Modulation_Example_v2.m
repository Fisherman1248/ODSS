...
cfg.pulseType = "rect";     % keep "rect" while validating the basis
cfg.SNRdB = 100;%25;             % waveform sample SNR for the channel example

% Single-path delay-scale channel. Change these values independently to
% study pure delay, pure scale, or their combination.
chan.h = 1;%exp(1j*pi/7);
chan.tau = 0;%2/cfg.Fs;        % two-sample propagation delay
chan.alpha = 1;%1.001;         % paper-style small time-scale factor
...
Ctest = GramNormalized;
Ctest(1:grid.Mtot+1:end) = 0;

[worstCorr, worstLinearIndex] = max(abs(Ctest(:)));
[worstRxIndex, worstTxIndex] = ind2sub(size(Ctest), ...
    worstLinearIndex);
...
fprintf("Channel and one-tap receiver sections were intentionally not run.\n");
% return;
...
%
%   Yhat = Dfull*TiMF*x + noise
%
% Unlike the one-tap receiver, this receiver retains all off-diagonal
% interference terms in Dfull.

Amatched = Dfull*TiMF;

% Approximate average noise variance after the matched-filter bank.
...
if cfg.SNRdB >= 80
    % Nearly noiseless validation: solve the full matched-filter system.
    xHatMatchedFull = Amatched\Yhat;
else
    % Source-domain LMMSE using the complete effective channel.
    xHatMatchedFull = ...
        (Amatched'*Amatched + ...
        noiseVarMatched*eye(grid.Mtot)) \ ...
        (Amatched'*Yhat);
end

berMatchedFull = mean( ...
    (real(xHatMatchedFull) >= 0) ~= (x >= 0));
...
if offDiagonalEnergyRatio > 0.1
    warning(['The sampled practical basis is not close to diagonal in this ' ...
        'configuration. A nonzero one-tap BER is therefore expected. ' ...
        'This does not invalidate the full LMMSE correctness test.']);
end
...
fprintf("  Channel full-LMMSE BER                : %.3e\n", berFull);
