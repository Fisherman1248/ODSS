...
cfg.txPulseType = "rect"; % transmit pulse: "rect" or "phydyas"
cfg.rxPulseType = "rect";     % receive pulse:  "rect" or "phydyas"
...
fprintf("  T = %.3f s, Fs = %.1f Hz, oversampling = %d\n", ...
    cfg.T, cfg.Fs, cfg.oversampling);
fprintf("  transmit pulse = %s, receive pulse = %s\n\n", ...
    cfg.txPulseType, cfg.rxPulseType);
...
[Gtx, basisInfo] = odssTransmitBasis( ...
    tTx, grid, cfg, cfg.txPulseType);
dt = 1/cfg.Fs;

basisEnergies = real(diag(dt*(Gtx'*Gtx)));
fprintf("Transmit-basis sampled energy range = [%.6f, %.6f]\n", ...
...
fprintf("Transmit-basis off-diagonal Frobenius ratio = %.3e\n", ...
    offDiagGramRatio);
fprintf("Transmit-basis maximum cross-correlation = %.2f dB\n", ...
    maxCrossCorrdB);
fprintf("Transmit-basis RMS cross-correlation = %.2f dB\n\n", ...
    rmsCrossCorrdB);
...
xlabel("Basis index");
ylabel("Basis index");
title(sprintf("Transmit-basis Gram matrix: %s (dB)", ...
    cfg.txPulseType));
...
title(sprintf("Representative transmit spectra: %s", ...
    cfg.txPulseType));
...
% For the selected receive pulse, its sampled approximation is a bank
% of inner products:
%
%   Yhat = dt*Grx^H*r.
%
...
[Grx, ~] = odssTransmitBasis( ...
    tRx, grid, cfg, cfg.rxPulseType);

receiveGram = dt*(Grx'*Grx);
receiveNorm = sqrt(max(real(diag(receiveGram)), eps));
receiveGramNormalized = receiveGram./(receiveNorm*receiveNorm.');
receiveOffDiagonalRatio = ...
    norm(receiveGramNormalized-diag(diag(receiveGramNormalized)), "fro") / ...
    norm(diag(diag(receiveGramNormalized)), "fro");

crossIdentity = dt*(Grx'*odssTransmitBasis( ...
    tRx, grid, cfg, cfg.txPulseType));
crossIdentityDiag = diag(diag(crossIdentity));
crossIdentityOffDiagonalRatio = ...
    norm(crossIdentity-crossIdentityDiag, "fro")^2 / ...
    max(norm(crossIdentityDiag, "fro")^2, eps);

fprintf("Receive-basis off-diagonal Frobenius ratio = %.3e\n", ...
    receiveOffDiagonalRatio);
fprintf("Tx/Rx identity cross-basis off-diagonal energy ratio = %.3e\n", ...
    crossIdentityOffDiagonalRatio);
...
if berOneTap > 0.05
    warning(['The diagonal one-tap approximation has a high BER for the ' ...
        'selected transmit/receive pulse pair. The full waveform and full ' ...
        'matched-filter-domain receivers remain the correctness references.']);
end
...
fprintf("  Transmit pulse                         : %s\n", cfg.txPulseType);
fprintf("  Receive pulse                          : %s\n", cfg.rxPulseType);
fprintf("  Mtot                                   : %d\n", grid.Mtot);
...
fprintf("  Basis RMS cross-correlation           : %.2f dB\n", ...
    rmsCrossCorrdB);
fprintf("  Receive-basis off-diagonal ratio       : %.3e\n", ...
    receiveOffDiagonalRatio);
fprintf("  Tx/Rx cross-basis off-diagonal energy  : %.3e\n", ...
    crossIdentityOffDiagonalRatio);
...
function [G, info] = odssTransmitBasis(t, grid, cfg, pulseType)
%ODSSTRANSMITBASIS Sample an ODSS transmit or receive basis.
%
% pulseType selects either the rectangular or the current PHYDYAS-windowed
% chirplet implementation. Passing the pulse explicitly keeps the transmit
% and receive designs independent.
%
...
% Successive q-adic compressions then expand the occupied bandwidth.
    ...
    basePulse = localBasePulse(tBase, T, f1, kappa, pulseType);
    ...
    for index = 1:grid.Mtot
        ...
        column(support) = q^(n/2) * ...
            localBasePulse(u(support), T, f1, kappa, pulseType) / ...
            baseNorm;
    ...
    info.pulseType = string(pulseType);
end
