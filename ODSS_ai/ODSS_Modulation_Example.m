...
% 4. The paper writes the basic chirplet frequencies as 1/sqrt(q) and
%    sqrt(q). To express them in hertz, this example multiplies both by W.
%    This implementation choice is stated explicitly and can be changed.
...
cfg.Fs = 1280;              % waveform sample rate in Hz
cfg.pulseType = "rect";     % "rect" or "phydyas"
...
fprintf("  T = %.3f s, Fs = %.1f Hz, pulse = %s\n\n", ...
    cfg.T, cfg.Fs, cfg.pulseType);
...
offDiagGramRatio = norm(GramNormalized-diag(diag(GramNormalized)), "fro") / ...
...
for ii = 1:numel(representative)
    ...
    plot(f(1:floor(nfftPlot/2)), ...
        20*log10(spectrum(1:floor(nfftPlot/2))+1e-12), ...
...
fprintf('\nStopped before channel and one-tap receiver tests.\n');
return;
...
tRxEnd = cfg.T + chan.tau + 4/cfg.Fs;
...
% M(n) follows the published typeset definition M(n) = ceil(q^n).
    ...
    grid.m = mIndex;
    grid.alpha = q.^nIndex;
    % grid.tau = mIndex./(q.^nIndex*W);
    T = 1.9;
    grid.tau = mIndex*T./q.^nIndex;
end
...
%   f1 = W/sqrt(q), f2 = W*sqrt(q).
% The paper prints normalized f1 = 1/sqrt(q), f2 = sqrt(q).
